...
EOF
  ...
  snapshots)
    echo "=== Snapshot Age Warnings ==="
    current_ts=$(date +%s)
    zfs list -H -t snapshot -o name,creation | while IFS=$'\t' read -r name creation; do
      base_dataset="${name%@*}"
      [[ ",$ignore_pools," == *",${base_dataset%%/*},"* ]] && continue
      snap_ts=$(date -d "$creation" +%s 2>/dev/null || echo 0)
      [[ "$snap_ts" -eq 0 ]] && continue
      age_days=$(((current_ts - snap_ts) / 86400))
      if [[ "$age_days" -ge "$snapshot_age_warn_days" ]]; then
        echo "$name is $age_days days old"
      fi
    done

    echo
    echo "=== Snapshot Count Warnings ==="
    zfs list -H -t snapshot -o name | cut -d@ -f1 | sort | uniq -c | while read -r count dataset; do
      [[ ",$ignore_pools," == *",${dataset%%/*},"* ]] && continue
